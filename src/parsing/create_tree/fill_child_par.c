...
static int	fill_child_entire_par(t_ast_nde *sib, t_ast_nde *raw_lft_child,  t_ast_nde *middle_child, t_ast_nde *raw_rght_child, t_ast_nde *token, t_ast_nde ***raw_child_sav)
{
	if (sib->end < token->start)
		add_sibling(copy_node(sib), &raw_lft_child->child, raw_child_sav[0]);
	else if (sib->start > token->end)
		add_sibling(copy_node(sib), &raw_rght_child->child, raw_child_sav[2]);
	else
		return (0);
	return (1);
}

static void	fill_child_overlap_par(t_ast_nde *sib, t_ast_nde *raw_lft_child, t_ast_nde *middle_child, t_ast_nde *raw_rght_child, t_ast_nde *token, t_ast_nde ***raw_child_sav)
{
	...
	t_ast_nde *token)
{
	...
	while (sib)
	{
		if (fill_child_entire_par(sib, raw_lft_child, middle_child, raw_rght_child, token, (t_ast_nde **[]){&raw_lft_child_sav,
			&middle_child_sav, &raw_rght_child_sav}))
			;
		else
			fill_child_overlap_par(sib, raw_lft_child, middle_child, raw_rght_child, token, (t_ast_nde **[]){&raw_lft_child_sav,
			&middle_child_sav, &raw_rght_child_sav});
		sib = sib->sibling;
	}
	if (raw_lft_child)
