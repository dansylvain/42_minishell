...
/*   Updated: 2024/03/09 09:46:26 by seblin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	fill_child_overlap(t_ast_nde *sib, t_ast_nde *raw_lft_child, t_ast_nde *raw_rght_child, t_ast_nde *token, t_ast_nde **raw_lft_sib_sav, t_ast_nde **raw_rght_sib_sav)
{
	...
	if (sib->start <= token->start && sib->end >= token->end)
	{
		if (sib->start < token->start)
		{
			...
			add_sibling(raw_overlap, &raw_lft_child->child, raw_lft_sib_sav);
		}
		if (sib->end > token->end)
		{
			raw_overlap = copy_node(sib);
			raw_overlap->start = token->end + 1;
			add_sibling(raw_overlap, &raw_rght_child->child, raw_rght_sib_sav);
		}
	}
}

void	fill_child(t_ast_nde *sib, t_ast_nde *raw_lft_child, t_ast_nde *raw_rght_child,
	t_ast_nde *token)
{
	...
	while (sib)
	{
		if (fill_child_entire(sib, raw_lft_child, raw_rght_child, token, &raw_lft_sib_sav, &raw_rght_sib_sav))
			...
			fill_child_overlap(sib, raw_lft_child, raw_rght_child, token, &raw_lft_sib_sav, &raw_rght_sib_sav);
		sib = sib->sibling;
	}
	if (raw_lft_child)
